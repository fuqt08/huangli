...
class Lunar
{
    ...
    public function getLunarGanzhiMonth()
    {
        ...
        if ($num[0] == '00' && $this->getLunarYearMonth()[0] == '腊月') {
            ...
            $tianGanOffset = array_keys($this->tianGan, $nT);
            $tianGan = $this->tianGan[($tianGanOffset[0] - 1 + 10) % 10];
        } else {
            $tianGan = $nT;
        }
        $jiNianMonthT = '';
    ...
    public function getJieQi()
    {
        ...
        //获取当前大致那个时令
        $theMonthJieQiData = $this->getJieQiData($year, $month[(int)$m]);
        //如果大致时令时间大于当前时间，那么本身时令就需要获取上一个月时令
        if (strtotime($theMonthJieQiData[1]) > $this->dateTime->getTimestamp()) {
            $data = [
                'current' => (int)$m == 1 ? $this->getJieQiData($year - 1, '20') : $this->getJieQiData($year, $month[(int)$m - 1]),//当前时令
            ...
            $data = [
                ...
                'next' => (int)$m == 12 ? $this->getJieQiData((int)$year + 1, '22') : $this->getJieQiData($year, $month[(int)$m + 1]),
            ];
        }
        return $data;
    }
